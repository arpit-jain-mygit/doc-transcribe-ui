...
| PRS-040 | 10 | Queue Orchestration Agent (dynamic balancing/prioritization) | Worker, API, UI | Throughput and fairness optimization | Lower wait time under mixed OCR/A-V load | Users: reduced wait under mixed load; Dev: adaptive orchestration hooks; Ops: lower queue contention; Product: better peak-time UX. | In Progress | Pending | Worker: added inflight requeue backoff+jitter and stale inflight cleanup guard (`f133018`) to prevent busy queue loops. UI: added queued timer, queue-load hints, and `Fair Scheduler` badge on processing card (`e37f91e`). API: pending explicit queue-health endpoint/telemetry contract. |
