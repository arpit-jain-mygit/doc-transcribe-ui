...
| PRS-048 | 5 | Multi-job async background processing with completion notifications | UI, API, Worker | Concurrent throughput and asynchronous user experience | Users can submit multiple jobs, leave the page, see live running-job count with per-job `%` progress, and still get completion visibility in history and email | Users: no waiting on single active job; live workload awareness; Dev: cleaner async orchestration; Ops: smoother queue utilization; Product: better completion and retention. | Planned | Pending | Planned scope: allow multi-submit queueing from UI, ensure independent background processing, show running job count + live per-job progress in UI, show completed cards in history, and send completion email per finished job. |
...
- 2026-02-19: Updated PRS-048 scope to include UI live running-job count and per-job `%` completion visibility while jobs process in background.
...
**Implementation summary**
1. Added deterministic cost projection service in API (`services/cost_guardrail.py`) with policy outputs (`ALLOW/WARN/BLOCK`).
2. Extended intake precheck contract with `estimated_effort`, `estimated_cost_band`, `policy_decision`, `policy_reason`, `projected_cost_usd`.
3. Enforced guardrail at upload-time in orchestrator so blocked jobs cannot bypass precheck.
4. Added UI precheck rendering for cost/effort/policy with decision color state.
...
1. Worker scheduling contract added via env:
   - `WORKER_SCHEDULER_POLICY=fifo|fair|adaptive`
   - `WORKER_SCHEDULER_MAX_CONSECUTIVE`
   - `WORKER_SCHEDULER_ACTIVE_DEPTH_MIN`
2. Worker dequeue logic now computes adaptive BRPOP target order from queue depths and dequeue streak.
3. API now exposes `/queue/health` (auth-protected) with queue depths, inflight counts, scheduler policy, and worker client count.
4. API contract capability now includes `queue_orchestration_enabled`.
5. UI polling consumes `/queue/health` while status is `QUEUED`, and surfaces scheduler policy + queue load hints in the processing card.
